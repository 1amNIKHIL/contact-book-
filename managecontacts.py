...
class ManageContactsFrame(Frame):
    ...
    def create_view_all_contacts_frame(self):
        ...
        self.create_update_delete_contact_frame)
    ...
    def add_button_click(self):
        ...
        if row is None:
            ...
            "Contact details of " + self.email_id_entry.get() + " email id is already added")
    ...
    def create_update_delete_contact_frame(self, event):
        ...
        width=20, values=('Greater Noida', 'Noida', 'Delhi', 'Gurgaon', 'Mumbai'))
        self.city_combobox.grid(row=3, column=1, pady=10)
        self.city_combobox.set(contact[3])

        self.profile_pic_label1 = Label(self.update_delete_contact_frame, text="Profile Pic: ")
        self.profile_pic_label1.grid(row=4, column=0)

        self.update_delete_contact_frame.image = ImageTk.PhotoImage(Image.open(contact[4]))
        self.profile_pic_label2 = Label(self.update_delete_contact_frame,
        image = self.update_delete_contact_frame.image)
        self.profile_pic_label2.grid(row=4, column=1, pady=10)

        self.style.configure('TButton', font=(NONE, 15), width=20)

        self.profile_pic_button = Button(self.update_delete_contact_frame, text="Choose your profile pic",
        command=self.profile_pic_button_click)
        self.profile_pic_button.grid(row=5, column=1, pady=10)
    ...
    def update_button_click(self):
        ...
        City = ?, ProfilePicPath = ? where EmailId = ?""", (
            self.name_entry.get(), self.phone_no_entry.get(), self.email_id_entry.get(),
            self.city_combobox.get(), self.profile_pic_path, self.old_email_id
        ))
        self.con.commit()
    ...
    def delete_button_click(self):
        if messagebox.askquestion("Confirmation Message", "Are you sure to delete?") == 'yes':
            ...
            self.con.commit()
            messagebox.showinfo("Sucess Message", "Contact details are deleted successfully")
            self.update_delete_contact_frame.destroy()
            self.create_view_all_contacts_frame()
